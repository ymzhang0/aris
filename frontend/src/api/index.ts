...
export const DEFAULT_WORKER_PROFILES: WorkerProfilesResponse = { current_profile: null, default_profile: null, profiles: [] };
export const DEFAULT_WORKER_RESOURCES: WorkerResourcesResponse = { computers: [], codes: [] };
...
export const aiidaClient = {
  ...
  async getWorkerStatus(): Promise<WorkerStatusResponse> {
    const { data } = await aiidaApi.get<WorkerStatusResponse>("/status");
    return data;
  }
  ,
  
    async getWorkerProfiles(): Promise<WorkerProfilesResponse> {
    try {
      const { data } = await aiidaApi.get<WorkerProfilesResponse>("/profiles");
      return data;
    } catch {
      return DEFAULT_WORKER_PROFILES;
    }
  }
  ,
  
    async switchWorkerProfile(profile: string): Promise<WorkerSwitchProfileResponse> {
    const { data } = await aiidaApi.post<WorkerSwitchProfileResponse>("/profiles/switch", { profile });
    return data;
  }
  ,
  
    async getWorkerResources(): Promise<WorkerResourcesResponse> {
    try {
      const { data } = await aiidaApi.get<WorkerResourcesResponse>("/resources");
      return data;
    } catch {
      return DEFAULT_WORKER_RESOURCES;
    }
  }
  ,
